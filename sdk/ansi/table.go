...
func TableForSlice(wr io.Writer, collection interface{}) error {
	...
	if cv.Kind() != reflect.Slice {
		return errors.New("table for slice; cannot iterate over non-slice collection")
	}
...
func Table(wr io.Writer, columns []string, rows [][]string) (err error) {
	if len(columns) == 0 {
		return errors.New("table; invalid columns; column set is empty")
	}
