...
func (e *Exception) GetClass() error { return e.Class }

// GetMessage returns the message.
func (e *Exception) GetMessage() string { return e.Message }

// GetMessage returns the message.
func (e *Exception) GetInner() error { return e.Inner }

// GetStackTrace returns the stack trace.
func (e *Exception) GetStackTrace() StackTrace { return e.StackTrace }
...
func (e *Exception) Decompose() map[string]interface{} {
	...
	values[KeyClass] = e.Class.Error()
	values[KeyMessage] = e.Message
	if e.StackTrace != nil {
		values[KeyStackTrace] = e.StackTrace.Strings()
	}
	if e.Inner != nil {
		if typed, isTyped := e.Inner.(*Exception); isTyped {
			values[KeyInner] = typed.Decompose()
		} else {
			values[KeyInner] = e.Inner.Error()
		}
	}
	return values
}
...
func (e *Exception) UnmarshalJSON(contents []byte) error {
	...
	if class, ok := values[KeyClass]; ok {
	...
	if message, ok := values[KeyMessage]; ok {
		if err := json.Unmarshal([]byte(message), &e.Message); err != nil {
			return New(err)
		}
	}

	if inner, ok := values[KeyInner]; ok {
	...
	if stack, ok := values[KeyStackTrace]; ok {
